...
if(server){
  ...
  apsim_file = file.path(main_dir, 'apsim.txt')
...
# WINDOWS
  ...
  flist = list.files(dirname(directory), full.names = TRUE, recursive = TRUE, pattern = '.apsim')
  if(any(str_detect(string = flist, pattern = '_yc_150'))) {
    flist <- flist[str_detect(string = flist, pattern = '_yc_150')]
  }
