...
missing_files <- readRDS("./vr_value_v2/Data/files_rds/missing_files.rds")
files_daily <- list.files('./vr_value_v2/Data/yc_output',full.names = T, recursive = T)
# files_daily <- list.files('S:/Bioinformatics Lab/germanm2/vr_value_v2_cluster/yc_output',full.names = T, recursive = T)
files_daily2 <- files_daily[basename(files_daily) %in% missing_files]
print(files_daily2)

for(file_n in files_daily2){
  # file_n <- files_daily2[1]
  print(file_n)
  ...
  possibleError <- tryCatch({
        daily_yc_dt <- readRDS(file_n) 
        # print(nrow(daily_yc_dt))
  }, error = function(e) e)

  #REAL WORK:if there is no error
  if(!inherits(possibleError, "error")){
    # return(rbindlist(all_summaries, fill = TRUE))
    if(!dir.exists("./vr_value_v2/Data/yc_output_temp")){dir.create("./vr_value_v2/Data/yc_output_temp")}
    saveRDS(daily_yc_dt, paste0("./vr_value_v2/Data/yc_output_temp/", basename(file_n)))
    # make_yearly_summary(file_n)
  }
}
