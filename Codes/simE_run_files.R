...
if(server){
  ...
  if(any(str_detect(string = flist, pattern = '_yc_'))) {
    ...
    if(test_small) {
      flist <- flist[str_detect(string = flist, pattern = '_yc_swim_160|_yc_swat_160|_yc_swat_212')] #OJO!!!!
    }
  }
  # print(flist)
...
# -----------------------------------------------------------------------
  ...
  flist = list.files(directory, full.names = TRUE, recursive = TRUE, pattern = '.apsim')
  if(any(str_detect(string = flist, pattern = '_yc_'))) {
    flist <- flist[str_detect(string = flist, pattern = '_yc_swim_160|_yc_swat_160')] #OJO!!!!
  }
