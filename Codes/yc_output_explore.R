library(data.table)
library(stringr)
# files_list <- list.files('S:/Bioinformatics Lab/germanm2/vr_value_v2_cluster/yc_output/', full.names = T) #CLUSTER
files_list <- list.files('S:/Bioinformatics Lab/germanm2/vr_value_v2/yc_output/', full.names = T) #SERVER
...
weather_dt <- readRDS('C:/Users/germanm2/Box Sync/My_Documents/vr_value_v2/Data/met_files/weather_z_cell37_dt.rds')
#---------------------------------------------------------------------------
#COMPARE SEQ VS CONT
files_list <- list.files('S:/Bioinformatics Lab/germanm2/vr_value_v2_cluster/yc_output/', full.names = T) #SERVER
id10_n = '137'

#COMPARE THE CONTINUOUS SIMULATION VS THE SEQUENTIAL
# COMPARE THE YIELD
ic_files <- list.files('S:/Bioinformatics Lab/germanm2/vr_value_v2_cluster/initial_conditions/', pattern = paste0(id10_n, '_'), full.names = TRUE)
ic_dt <- data.table()
for(file_n in ic_files){
  ic_dt <- rbind(ic_dt, readRDS(file_n))}

yc_files <- list.files('S:/Bioinformatics Lab/germanm2/vr_value_v2_cluster/yc_output/', pattern = paste0(id10_n, '_'), full.names = TRUE)
yc_dt <- data.table()
