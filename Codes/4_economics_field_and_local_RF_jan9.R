...
full_fields_dt2 <- full_fields_dt[,.(.N), by = .(id_10, id_field)][,-'N'] #one row by field
table(full_fields_dt2[,.N, .(id_10)]$N)

full_fields_dt2[,N := .N, .(id_10)]
full_fields_wide_dt <- dcast(full_fields_dt2, id_10 + N ~ id_field, value.var = "id_field")
names(full_fields_wide_dt)[3:6] <- paste('field', names(full_fields_wide_dt)[3:6], sep = '_')
...
process_field_economics <- function(j){
  ...
  # LOCAL COMBINING INFORMATION FROM BOTH FIELDS
  ...
  ss_var_local <-  ss_var[!ss_var %in% c("dul_dep", "ll15_dep","whc")] #this is captured by the mukey
  crop_varb
  training_set_yield <- ic_field_dt_both[z %in% training_z, c('mukey','area_ha', 'z','N_fert', 'Yld', 'P', no_cost_var_local, ss_var_local), with = FALSE] #this is the first time we are training in this field
  table(training_set_yield$z)
  training_set_yield_aggregated  <- aggregate_by_area(data_dt = training_set_yield, variables = c('P', 'Yld'), weight = 'area_ha', by_c = c('N_fert','z'))
  table(training_set_yield_aggregated$z)
...
perfomances_dt2[, .(Yld =  mean(Yld),
                    ...
                    overpred = sum(overpred)/.N), by = .( model, tech)][order(-P)]
