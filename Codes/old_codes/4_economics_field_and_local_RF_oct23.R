...
process_field_economics <- function(j){
  ...
  training_set_yield <- training_set_yield[,Yld_max_z := max(Yld), by = .(mukey, z, prev_crop)] %>% 
    .[Yld_max_z > 2000 ] %>% .[,-'Yld_max_z'] #remove bad data. Assumption: Trials would be discarded or mukey cleaned
  ...
  training_set_eonr_aggregated <- aggregate_by_area(data_dt = training_set_yield, 
                                  ...
                                  .[, .SD[ N_fert == min( N_fert)], by = .(z, prev_crop)] %>%
  ...
  training_set_eonr <- training_set_yield[, .SD[ P == max( P)], by = .(mukey, z, prev_crop)] %>%
    .[, .SD[ N_fert == min( N_fert)], by = .(mukey, z, prev_crop)] %>%
    setnames('N_fert', 'eonr') %>%
