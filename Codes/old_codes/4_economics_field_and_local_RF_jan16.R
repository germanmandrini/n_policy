...
process_field_economics <- function(j){
  ...
  # field_n <- id_10_info$id_field[1]
    ...
    z_even = z_odd+1
    z_select <- if(field_n_loop %in% c(1,3)){z_odd} else{z_even}
    ...
    ic_field_dt_both <- rbind(ic_field_dt_both, ic_field_dt[,id_field := field_n_loop])
  } #end of field_n_loop loop
  ...
  formula_lm <- paste0('eonr ~ ', paste(no_cost_var_local, collapse = ' + '))
  formula_lm <- 'eonr ~ rain_60'
  var_remove <- 'start'
  while(length(var_remove)==1){
    print(var_remove)
    reg_lm1_ur <- lm(data = training_set_eonr_aggregated, formula = as.formula(formula_lm))
    summary(reg_lm1_ur)
    
    sum_reg_mt <- summary(reg_lm1_ur)$coefficients
  ...
  #===================================================================================================================
  # 9) ONE BEST RATE by MUKEY
  # GET THE RECOMMENDATION FOR THE Z11-30 FOR EACH MUKEY
  # Need to train with aggregated data. We will aggregate the data and select the best N rate for the whole field
  training_set_yield_aggregated  <- aggregate_by_area(data_dt = training_set_yield, variables = c('P', 'Yld'), weight = 'area_ha', by_c = c('N_fert', 'mukey'))
  
  local_best_rate_model_mukey <- training_set_yield_aggregated[, .SD[ P == max( P)], by = mukey] %>%
    setnames('N_fert', 'eonr_pred') %>% .[, c('mukey', 'eonr_pred')]
  ...
  # PERFORMANCE EVALUATION
  testing_set <- merge(ic_field_dt[!z %in% training_z],
                       local_best_rate_model_mukey, by = 'mukey') %>% #here we join back the predictions with the data with all the N rates and then filter the N rate = to the predicted to measure testing
    .[,eonr_pred := ifelse(eonr_pred <10, 10, ifelse(eonr_pred > 330, 330, eonr_pred))] %>%
    .[N_fert == eonr_pred] %>%
    .[,c("mukey", "z", "id_10", "area_ha", "Yld", "leach_n2", "n_deep_v5", "N_fert", 'P')]
  
  small_list[[length(small_list)+1]] <- testing_set[,model := 'Local_onebestrate'][,tech := 'VR']
  #===================================================================================================================
  # 10) PREDICT WITH LOCAL RF 1 - UR 
  # GET THE RECOMMENDATION FOR THE Z11-30 FOR EACH MUKEY
  # Need to train with aggregated data. We will aggregate the data and select the best N rate for the whole field
  #now we train the model
  ...
  #Get the predictions for the prediction set aggregated
  prediction_set_aggregated %>% .[,eonr_pred := predictrf_quantile(model = rf1_local_ur, data = ., quant = 0.7)]
  ...
  # PERFORMANCE EVALUATION
  testing_set <- ic_field_dt[!z %in% training_z] %>% 
  .[,eonr_pred := predictrf_quantile(model = rf1_local_vr, data = ., quant = 0.7)] %>%
  .[,eonr_pred := ifelse(eonr_pred <10, 10, ifelse(eonr_pred > 330, 330, eonr_pred))] %>%
  .[N_fert == eonr_pred] %>% 
  .[,c("mukey", "z", "id_10", "area_ha", "Yld", "leach_n2", "n_deep_v5", "N_fert", 'P' )] %>% .[order(mukey, z)]
  ...
  prediction_set_aggregated %>% .[,eonr_pred := predictrf_quantile(model = rf2_local_ur, data = ., quant = 0.7)]
  ...
  # PERFORMANCE EVALUATION
  testing_set <- ic_field_dt[!z %in% training_z] %>% 
    .[,eonr_pred := predictrf_quantile(model = rf2_local_vr, data = ., quant = 0.7)] %>%
  .[,eonr_pred := ifelse(eonr_pred <10, 10, ifelse(eonr_pred > 330, 330, eonr_pred))] %>%
  .[N_fert == eonr_pred] %>% 
  .[,c("mukey", "z", "id_10", "area_ha", "Yld", "leach_n2", "n_deep_v5", "N_fert", 'P' )] %>% .[order(mukey, z)]
  ...
  training_set_yield_aggregated[, Yld_response := max(Yld) - min(Yld), by = .(z, id_field)]
  training_set_yield_aggregated <- training_set_yield_aggregated[Yld_response > 500]
  quadratic_dt <- training_set_yield_aggregated[,list(intercept=coef(lm(Yld~N_fert + I(N_fert^2)))[1], 
                                                      coef1=coef(lm(Yld ~ N_fert + I(N_fert^2)))[2],
  ...
  # GET THE RECOMMENDATION FOR THE Z11-30 FOR EACH MUKEY
  # Need to train with aggregated data. We will aggregate the data and select the best N rate for the whole field
  training_set_yield_aggregated  <- aggregate_by_area(data_dt = training_set_yield, variables = c('P', 'Yld'), weight = 'area_ha', 
                                                      by_c = c('N_fert')) #again because we removed the [Yld_response > 500]
  ...
  small_list[[length(small_list)+1]] <- testing_set[,model := '9_Local_onebestrate_comb'][,tech := 'UR']
  ...
  training_set_eonr_aggregated <- aggregate_by_area(data_dt = training_set_yield, 
                                                    variables = do_aggregate, 
                                                    weight = 'area_ha', by_c = c('id_field','z', 'N_fert')) %>% data.table() %>%
    .[, .SD[ P == max( P)], by = .(id_field, z)] %>%
    setnames('N_fert', 'eonr') %>%
    .[, c('id_field','eonr', no_cost_var_local, ss_var_local), with = FALSE]
  ...
  small_list[[length(small_list)+1]] <- testing_set[,model := '10_reg4_local_comb'][,tech := 'UR']
  ...
  # Need to train with aggregated data. We will aggregate the data and select the best N rate for the whole field
  #now we train the model
  rf1_local_ur <- randomForest(eonr ~ ., data = training_set_eonr_aggregated[,c('id_field','eonr', no_cost_var_local), with = FALSE], importance = TRUE, 
                               ntree=50)
  # varImpPlot(rf1_local_ur, type=2)
  # plot(model1_local_ur)
  ...
  prediction_set_aggregated[,id_field :=  field_info$id_field] #need the id_field because now it is a variable of the model
  prediction_set_aggregated %>% .[,eonr_pred := predictrf_quantile(model = rf1_local_ur, data = ., quant = 0.7)]
  #---------------------------------------------------------------------------
  # PERFORMANCE EVALUATION
  #Predictions were made for the aggregated data, eates are obtained and testing is evaluated with data not aggregated
  ...
  training_set_eonr <- training_set_yield[, .SD[ P == max( P)], by = .(id_field, mukey, z)] %>%
    setnames('N_fert', 'eonr') %>%
    .[, c('id_field','mukey','eonr', no_cost_var_local, ss_var_local), with = FALSE]
  
  sapply(training_set_eonr, class)
  # Create a Random Forest model with default parameters
  rf1_local_vr <- randomForest(eonr ~ ., data = training_set_eonr[,c('id_field', 'eonr', 'mukey' ,no_cost_var_local), with = FALSE], importance = TRUE, 
                               ntree=50)
  # varImpPlot(rf1_local_vr, type=2)
  # plot(model1_local_vr)
  ...
  rf2_local_ur <- randomForest(eonr ~ ., data = training_set_eonr_aggregated[,c('id_field', 'eonr' ,no_cost_var_local, ss_var_local), with = FALSE], importance = TRUE, 
                               ntree=50)
  ...
  # Create a Random Forest model with default parameters
  rf2_local_vr <- randomForest(eonr ~ ., data = training_set_eonr[,c('id_field', 'eonr', 'mukey' ,no_cost_var_local, ss_var_local), with = FALSE], importance = TRUE, 
                               ntree=50)
  # varImpPlot(rf2_local_vr, type=2)
...
saveRDS(perfomances_dt2, "./vr_value_v2/Data/files_rds/perfomances_dt2.rds")
