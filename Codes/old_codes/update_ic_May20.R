update_ic <- function(base_doc, instructions_tmp, initial_conditions){  
  ...
  file_name <- paste0('./vr_value/Data/initial_conditions/cell_', instructions_tmp$id_10, '/',paste(instructions_tmp[,-'N'], collapse = '_'),'.rds')
  file_name <- gsub(pattern = '_YC', replacement = '_stab', file_name)
  file_name <- gsub(pattern = paste0('_', instructions_tmp$z, '_'), replacement = '_A1_', file_name) #only A1 is run as stabilization period
  ...
  initial_conditions_tmp <- initial_conditions_tmp[mukey == instructions_tmp$mukey &
                                              rotation == instructions_tmp$rotation &
                                               year == year_n & day == 366 & month == 12]
