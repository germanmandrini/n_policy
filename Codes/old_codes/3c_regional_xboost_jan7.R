...
# Make predictions
preds <- predict(xgb_model, trainTask)
...
train <-  TrainSet_eonr3[,c("eonr", no_cost_varb_trf, ss_varb), with = FALSE]
test <- ValidSet_eonr3[,c("eonr", no_cost_varb_trf, ss_varb), with = FALSE] 
...
train_task_data <- rbind(train, test)
size <- nrow(train_task_data)
train_ind <- seq_len(nrow(train))
validation_ind <- seq.int(max(train) + 1, size)
# 
# 
