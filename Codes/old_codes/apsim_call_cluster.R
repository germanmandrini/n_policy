...
folder_name <- '/projects/aces/germanm2/'
setwd(folder_name)
...
library(parallel)
library(XML)
i_n  <- commandArgs(trailingOnly = TRUE)
i_n <- as.numeric(i_n)
# i_n = 243
...
# for(id10_n in id_10_seq){
  id10_n = id_10_seq[i_n]
  print(id10_n)
  start1 <- Sys.time()

  #Get the directory to save the run

  # directory <- paste('C:/apsim_temp/', Sys.info()["nodename"],'/vr_value_v2/cell', id10_n, sep = '') #CPSC
  ...
  start2 <- Sys.time()
  source('./vr_value_v2/Codes/apsim_create_files_11yrs.R')
...
if(FALSE){ 
  # print(paste('CREATE', round(difftime(Sys.time(), start2, units = "mins"),2), 'mins'))
   ...
   # usr/bin/mono /usr/local/APSIMClassic/Model/Apsim.exe ./vr_value_v2/apsim_temp/cell152/152*
