...
for(z_n in 1:30){
  ...
  for(level_n in bal_seq){
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'staticmrtn'][,policy := paste0('bal_', level_n)]
    #===================================================================================================================
    # 2) RFHIGH
    # GET THE RECOMMENDATION FOR THE Z11-30 FOR EACH id_field
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'rfhigh'][,policy := paste0('bal_', level_n)]
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'rfranger'][,policy := paste0('bal_', level_n)]
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'expost'][,policy := paste0('bal_', level_n)]
    #===================================================================================================================
  } #end of balance loop
  ...
  for(level_n in red_seq){
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'staticmrtn'][,policy := paste0('red_', level_n)]
    #===================================================================================================================
    # 2) RFHIGH
    # GET THE RECOMMENDATION FOR THE Z11-30 FOR EACH id_field
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'rfhigh'][,policy := paste0('red_', level_n)]
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'rfranger'][,policy := paste0('red_', level_n)]
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'expost'][,policy := paste0('red_', level_n)]
    #===================================================================================================================
  ...
  for(level_n in lag_seq){
    ...
    static_mrtn_dt  <- static_data[P_diff >= -level_n] %>% # get out of the flat zone: lowest rate withing diff_n usd difference from max Profit
      .[, .SD[ N_fert == min( N_fert)], by = .(region)] %>%
      .[,.(region, eonr_pred = N_fert)] %>%
      .[order(region)]
    ...
    training_eonr_dt  <- training_eonr_dt[P_diff >= -level_n] %>% # get out of the flat zone: lowest rate withing 10 usd difference from max Profit
      .[, .SD[ N_fert == min( N_fert)], by = .(id_10, id_field, z)] 
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'staticmrtn'][,policy := paste0('lag_', level_n)]
    #===================================================================================================================
    # 2) RFHIGH
    # GET THE RECOMMENDATION FOR THE Z11-30 FOR EACH id_field
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'rfhigh'][,policy := paste0('lag_', level_n)]
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'rfranger'][,policy := paste0('lag_', level_n)]
    ...
    results_list[[length(results_list)+1]] <- evaluation_set_tmp[,NRS := 'expost'][,policy := paste0('lag_', level_n)]
    #===================================================================================================================
  } #end of lag loop
  perfomances_z_tmp <- rbindlist(results_list)
  
  saveRDS(perfomances_z_tmp, paste0("./n_policy_box/Data/files_rds/field_performances_tmp/field_performance_", z_n,'.rds'))
...
files_path <- list.files("./n_policy_box/Data/files_rds/field_performances_tmp", full.names = TRUE)
