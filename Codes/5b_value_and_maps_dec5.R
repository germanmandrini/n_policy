...
unique(perfomances_dt$model)
perfomances_dt <- perfomances_dt[model %in% c("1", 
                                              "rf1_q70", "rf2_q70", 
                                              "6",  "7", "8", "9", "10", "11", "12") ]
  
perfomances_dt[model == 'rf1_q70' & tech == 'UR', model := '2']
perfomances_dt[model == 'rf1_q70' & tech == 'VR', model := '3']
perfomances_dt[model == 'rf2_q70' & tech == 'UR', model := '4']
perfomances_dt[model == 'rf2_q70' & tech == 'VR', model := '5']

unique(perfomances_dt$model)

# perfomances_dt[,model := as.numeric(model)]

summary(perfomances_dt[,.(area_ha = sum(area_ha)), by = .(id_10, id_field, model, tech, z)]$area_ha)

#-------------------------------------------------------------------------
# AGGREGATE THE DATA TO CELL X Z LEVEL CONSIDERING THE AREA
names(perfomances_dt)
do_not_aggregate = c('id_10', 'region','model', 'tech', 'z')
...
perfomances_dt3 <- perfomances_dt2[, .(Yld =  mean(Yld),
                                       ...
                                       N_fert = mean(N_fert),
                                       P = mean(P), 
                                       area_ha = mean(area_ha)), by = .(region, id_10, model, tech)] #field level (z and prev_crop are out)
...
perfomances_dt4 <- aggregate_by_area(data_dt = perfomances_dt3, variables = c("Yld", "leach_n2", "leach_ext", "N_fert","P"), 
                                         weight = 'corn_avg_ha', by_c = c('model', 'tech')) #state level, weighted by corn_ha
perfomances_dt4[order(-P)]
...
(p1 <- ggplot(perfomances_dt4, aes(x = model, y =  Yld, fill = tech))+
    ...
    scale_y_continuous(limits=c(min(perfomances_dt4$Yld)-100,max(perfomances_dt4$Yld)+200),oob = rescale_none)+
    ggtitle('Yield')+
    ylab('Yield (kg/ha)') +
...
ggsave(grid.arrange(p1, p2, p3, ncol = 1), filename = "./vr_value_v2/Data/figures/state_total_barchart1.jpg")

ggsave(grid.arrange(p4, p5, ncol = 1), filename = "./vr_value_v2/Data/figures/state_total_barchart2.jpg")
#---------------------------------------------------------------------------
...
ggsave(gridExtra::grid.arrange(p1, p2, p3, p4, p5, ncol=2), filename = "./vr_value_v2/Data/figures/state_total_1vs4.jpg")

#---------------------------------------------------------------------------
# RMSE
paired <- merge(perfomances_dt[, .(id_10, id_field, region, mukey,z, model, N_fert)],
                perfomances_dt[model == 12, .(id_10, id_field, region, mukey,z, N_fert)],
                by = c('id_10', 'id_field', 'region', 'mukey', 'z'), suffixes=c("", "_12"))
...
model_minimum_regional2 <- merge(model_minimum_regional, mrtn_dt[,-'MRTN_Rate_lbN_ac'], by = c('region'))
...
model_minimum_regional2[order(-region)]
...
value_sf <- merge(grid10_tiles_sf7, value_dt[,.(id_10, leach_n2_cell_1,leach_val, leach_ext_cell_1)], by = 'id_10', all.x = T)
...
st_write(value_sf, "./vr_value_v2/Data/Grid/grid10_leach_hotspots.shp", delete_dsn=TRUE)
...
# tmap_save(p1, "./vr_value_v2/Data/figures/baseline_characterization_map.jpg")
...
value_sf <- merge(grid10_tiles_sf7, value_dt[,.(id_10, P)], by = 'id_10', all.x = T) %>%
...
value_sf <- merge(grid10_tiles_sf7, value_dt[,.(id_10, mkt_value)], by = 'id_10', all.x = T) %>%
  dplyr::mutate(method = 'ex_ante')
...
tmap_save(tmap_arrange(p1, p2, p3, nrow = 1), "./vr_value_v2/Data/figures/techonology_characterization_map.jpg", width = 10.5, height = 4)
...
best_method_dt[,.N, by = .(model)][order(-N)]

value_sf <- merge(grid10_tiles_sf7, best_method_dt[,.(id_10, model)], 
                   by = 'id_10', all = T)
...
(plot_n2 <- ggplot() +
    ...
    geom_line(data = ic_field_plot, aes(x = N_fert, y = Yld, group = interaction(z))) +
    # scale_size_manual(values=c(rep(2, 11), 4)) +
    ## scale_color_manual(values=colors_sample)+
    labs(y = 'Yield (kg/ha)',
         x = 'N rate (kg/ha)')+
...
perfomances_champaign_dt <- perfomances_dt[id_10 %in% unique(dplyr::filter(grid10_tiles_sf7, county_name == 'Champaign')$id_10) & model == 12]
perfomances_champaign_dt[,prev_crop:= as.character(prev_crop)]

(plot_n3 <- ggplot() + 
    geom_density(data = perfomances_champaign_dt, aes( x= N_fert,  y = ..density.., fill = linetype = prev_crop), alpha = 0.4)+
...
(plot_n4 <- ggplot() + 
    geom_density(data = perfomances_champaign_dt, aes( x= leach_n2,  y = ..density.., fill = linetype = prev_crop), alpha = 0.4)+
...
ggsave(grid.arrange(plot_n1, plot_n2, plot_n3, plot_n4, nrow = 2), 
       filename = "./vr_value_v2/Data/figures/yield_curve_example.jpg", width = 8, height =  5.67)
