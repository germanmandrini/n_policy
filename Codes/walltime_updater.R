...
files_all <- list.files('./vr_value_v2/Data/yc_output', recursive = T, pattern = '.rds')
# files_all <- list.files('S:/Bioinformatics Lab/germanm2/vr_value_v2_cluster/yc_output/', recursive = T, pattern = '.rds')

id_10_runned <- unique(sapply(strsplit(as.character(files_all), split="_"), "[", 1) )

id_10_walltime_dt <- readRDS("./vr_value_v2/Data/files_rds/id_10_walltime_dt.rds")
...
write.table(id_10_walltime_dt[,.(id_10, dur)], './vr_value_v2/id_10_walltime.txt', row.names = F, col.names = F)

if(FALSE){ #send again files while still running
  
  id_10_walltime_dt_sent <- fread('./vr_value_v2/id_10_walltime.txt', col.names = c('id_10', 'dur'))
  ...
  write.table(id_10_walltime_dt[,.(id_10, dur)], './vr_value_v2/id_10_walltime2.txt', row.names = F, col.names = F)
...
if(FALSE){ #re run id10 that didn't pass the daily to yearly
  grid10_soils_dt4 <- readRDS("./vr_value_v2/Data/Grid/grid10_soils_dt4.rds")
  files_yearly <- list.files("./vr_value_v2/Data/yc_output_summary", recursive = T)
  library(data.table)
