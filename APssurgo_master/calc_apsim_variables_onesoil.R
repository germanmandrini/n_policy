...
calc_apsim_variables <- function(horizons_cell_dt, region_n){
  ...
  for(mukey_n in mukey_seq){
    ...
    if(region_n == 1){ #South
      ...
      horizon$FBiom = c(0.025, 0.015, 0.01, 0.005, 0.001, 0.001,0.001, 0.001, 0.001,0.001)
      horizon$FInert = c(0.75, 0.8, 0.8, 0.85, 0.85, 0.9, 0.98,0.98, 0.99, 0.99)
      
    }else if(region_n == 2){ #Central
      horizon$FBiom = c(0.06, 0.04, 0.03, 0.02, 0.01, 0.005,0.005, 0.005, 0.001,0.001)
      horizon$FInert = c(0.6, 0.65, 0.7, 0.7, 0.75, 0.75, 0.80,0.80, 0.92, 0.98)
      
    }else if(region_n == 3){ #North
      horizon$FBiom = c(0.08, 0.05, 0.04, 0.025, 0.02, 0.01,0.01, 0.005, 0.005, 0.001)
      horizon$FInert = c(0.5, 0.53, 0.55, 0.6, 0.65, 0.7, 0.80,0.80, 0.92, 0.98)
    }
