...
calc_apsim_variables <- function(horizons_cell_dt, region_n){
  ...
  for(mukey_n in mukey_seq){
    ...
    if(region_n == 1){ #South
      ...
      horizon$FBiom = c(0.060, 0.050, 0.045, 0.04, 0.02, 0.01, 0.01, 0.01, 0.001, 0.001)
      horizon$FInert = c(0.6, 0.65, 0.7, 0.7, 0.75, 0.75, 0.80,0.80, 0.92, 0.98)
      
    }else if(region_n == 2){ #Central
      horizon$FBiom = c(0.050, 0.030, 0.030, 0.030, 0.020, 0.01, 0.01, 0.005, 0.001, 0.001)
      horizon$FInert = c(0.6, 0.65, 0.7, 0.7, 0.75, 0.75, 0.80,0.80, 0.92, 0.98)

    }else if(region_n == 3){ #North
      horizon$FBiom = c(0.090, 0.060, 0.060, 0.055, 0.050, 0.040, 0.040, 0.02, 0.01, 0.001)
