...
calc_apsim_variables <- function(horizons_cell_dt, region_n){
  ...
  for(mukey_n in mukey_seq){
    ...
    if(region_n == 1){ #South
      ...
      horizon$FBiom = c(0.03, 0.025, 0.02, 0.015, 0.015, 0.01,0.005, 0.001, 0.001,0.001)
      horizon$FInert = c(0.5, 0.55, 0.6, 0.75, 0.8, 0.85, 0.9,0.95, 0.97, 0.99)
      
    }else if(region_n == 2){ #Central
      horizon$FBiom = c(0.07, 0.06, 0.05, 0.035, 0.015, 0.01,0.005, 0.005, 0.001,0.001)
      ...
      horizon$FBiom = c(0.11, 0.09, 0.07, 0.05, 0.03, 0.01, 0.01, 0.005, 0.001, 0.001)
